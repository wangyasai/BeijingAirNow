...
var degree = ["0-15：世界卫生组织标准","0-35：中国标准","0-50：国家标准","50-100：良","100-150：一级污染","150-200：二级污染","200-300：三级污染","300-500：四级污染"]
...
var yearDays = [339,365,365,366,365,360];
var images = [];
var bfPosX  = [];
var bfPosY  = [];
var afPosX  = [];
...
//everyday pm2.5 bar
function SkyBar(x, y, value, bfX, afX,images){
...
function showImg(){
    for(var j = 0; j<6;j++){
        for(var i = 0; i<yearDays[j]; i++){   
            if(mouseX >  bfPosX[i] && mouseX < bfPosX[i]+barW && mouseY > bfPosY[0] && mouseY < bfPosY[0]+ barH && j == 0){
                image(images13[i],mouseX,bfPosY[0]+barH*0.7+80,120,160);
            }else if( mouseX >  bfPosX[i] && mouseX < bfPosX[i]+barW &&mouseY > bfPosY[1] && mouseY < bfPosY[1]+ barH && j == 1){
                image(images14[i],mouseX,bfPosY[1]+barH*0.7+80,120,160);
            }else if( mouseX >  bfPosX[i] && mouseX < bfPosX[i]+barW &&mouseY > bfPosY[2] && mouseY < bfPosY[2]+ barH && j == 2){
                image(images15[i],mouseX,bfPosY[2]+barH*0.7+80,120,160);
            }else if( mouseX >  bfPosX[i] && mouseX < bfPosX[i]+barW &&mouseY > bfPosY[3] && mouseY < bfPosY[3]+ barH && j == 3){
                image(images16[i],mouseX,bfPosY[3]+barH*0.7+80,120,160);
            }else if( mouseX >  bfPosX[i] && mouseX < bfPosX[i]+barW &&mouseY > bfPosY[4] && mouseY < bfPosY[4]+ barH && j == 4){
                image(images17[i],mouseX,bfPosY[4]+barH*0.7+80,120,160);
            }else if( mouseX >  bfPosX[i] && mouseX < bfPosX[i]+barW &&mouseY > bfPosY[5] && mouseY < bfPosY[5]+ barH && j == 5){
...
function bfBox(posY, n, lerp,year) {
        for(var i = 0; i< yearDays[n]; i++){  
            if (mouseX > bfPosX[i] && mouseX < bfPosX[i]+barW && mouseY > posY && mouseY < posY+ barH) {
                ...
                textSize(12);
                noStroke();
                text(year + "/" + day, mouseX - 60, posY+barH*0.7 + 170);
...
function drawLegend(posY) {
    ...
    textSize(12);
    textAlign(RIGHT);
    text("好", posX - 2, posY+5);
    textAlign(LEFT);
    text("糟糕", posX + 8 * w_ + 2, posY+5);
